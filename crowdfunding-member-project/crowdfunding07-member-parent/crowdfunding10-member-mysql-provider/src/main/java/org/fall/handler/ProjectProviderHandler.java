...
public class ProjectProviderHandler {
    ...
    public ResultEntity<String> updateProjectStatusRemote(@RequestParam("projectName") String projectName, @RequestParam("supportPrice") Integer supportPrice) {
        // 调用本地service进行更新
        try {
            projectService.updateProjectStatus(projectName, supportPrice);
            return ResultEntity.successWithoutData();
        } catch (Exception e) {
            e.printStackTrace();
            return ResultEntity.failed(e.getMessage());
