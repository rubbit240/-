...
public class ProjectServiceImpl implements ProjectService {
    ...
    @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = Exception.class)
    @Override
    public void updateProjectStatus(String projectName, Integer supportPrice) {
        ProjectPO projectPO = projectPOMapper.selectProjectByProjectName(projectName);
        if (null != projectPO) {
            Integer money = projectPO.getMoney();
            Long supportmoney = projectPO.getSupportmoney();

            supportmoney = supportmoney + supportPrice;
            int status = 0;
            if (supportmoney >= money) {
                status = 2;
            } else {
                status = 1;
            }
            projectPOMapper.updateProjectStatusAndSupportmoney(projectPO.getId(), status, supportmoney);
        }
    }
